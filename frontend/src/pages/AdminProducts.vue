...
const createEmptyProduct = () => ({
  name: '', brand: '', category: 'Sneakers', price: '',
  compare_price: '', sku: '', description: '', tags: '',
  images: [], newImageUrl: '',
  colors: [], newColor: '',
  sizes: [], newSize: '',
...
const openProductModal = (product = null) => {
  if (product) {
    ...
    productForms.value = [{
      name: product.name,
      brand: product.brand || '',
      category: product.category?.name || product.category_name || 'Sneakers',
      price: product.price,
...
const removeImage = (form, i) => form.images.splice(i, 1)
...
const submitProducts = async () => {
  try {
    for (const form of productForms.value) {
      const productData = {
        name: form.name,
        brand: form.brand,
      ...
      if (editingProduct.value) {
        await update(editingProduct.value.id, productData)
      } else {
...
// Load data
onMounted(async () => {
  await Promise.all([fetchProductsData(), fetchStats()])
})
...
// Computed
const totalProducts = computed(() => stats.value.total || products.value.length)
const activeProducts = computed(() => stats.value.active || 0)
const outOfStockProducts = computed(() => stats.value.out_of_stock || 0)
const lowStockProducts = computed(() => stats.value.low_stock || 0)
...
const getProductImage = (product) => {
  return product.images?.[0] || product.main_image || 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=200&q=80'
}
                ...
                <tr v-for="product in products" :key="product.id" class="hover:bg-gray-50 transition-colors">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-12 w-12 border border-gray-100 rounded-lg overflow-hidden bg-white">
                        <img :src="getProductImage(product)" :alt="product.name" class="h-12 w-12 object-cover object-center" />
                      </div>
                      <div class="ml-4">
                        <div class="text-sm font-medium text-gray-900 truncate max-w-[200px]" :title="product.name">{{ product.name }}</div>
                        <div class="text-sm text-gray-500">ID: #{{ product.id?.slice(-4) || '0000' }}</div>
                      </div>
                    </div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex justify-end gap-2">
                       <button @click="openProductModal(product)" class="text-gray-500 hover:text-blue-600 p-1.5 rounded-lg transition-colors hover:bg-blue-50" title="Edit">
                      ...
                      <button @click="deleteProduct(product)" class="text-gray-500 hover:text-red-600 p-1.5 rounded-lg transition-colors hover:bg-red-50" title="Delete">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <div v-if="showProductModal" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto py-8 px-4" style="background:rgba(0,0,0,0.5);backdrop-filter:blur(4px)">
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl z-10">
              <div>
                <h2 class="text-lg font-bold text-gray-900">{{ editingProduct ? 'Edit Product' : 'Add Products' }}</h2>
                <p class="text-xs text-gray-500 mt-0.5">{{ editingProduct ? 'Update product details' : 'Fill in the details below. Add multiple products at once.' }}</p>
              </div>
              ...
              <div v-for="(form, idx) in productForms" :key="idx" class="border border-gray-200 rounded-xl p-5 relative bg-gray-50/50">
                ...
                <div class="grid grid-cols-3 gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <input v-model="form.compare_price" type="number" min="0" placeholder="2000000" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white" />
                  </div>
                  <div>
                ...
                <div class="grid grid-cols-3 gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <select v-model="form.is_active" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white">
                ...
                <!-- Image URLs -->
                <div class="mb-3">
                  <label class="block text-xs font-medium text-gray-700 mb-1">Image URLs</label>
                  <div class="flex gap-2 mb-2">
                    <input v-model="form.newImageUrl" @keyup.enter="addImageUrl(form)" type="url" placeholder="https://example.com/image.jpg" class="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white" />
                    <button @click="addImageUrl(form)" type="button" class="px-3 py-2 bg-gray-900 hover:bg-gray-700 text-white rounded-lg text-sm font-medium transition-colors">+ Add</button>
                  </div>
                  <div class="flex flex-wrap gap-2">
                    <div v-for="(img, imgIdx) in form.images" :key="imgIdx" class="relative inline-block">
                      <img :src="img" class="h-16 w-16 object-cover rounded-lg border border-gray-200" />
                      <button @click="removeImage(form, imgIdx)" type="button" class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center text-xs transition-colors">
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex items-center justify-between">
              <button v-if="!editingProduct" @click="addProductForm" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-gray-900 border border-gray-300 hover:border-gray-400 bg-white px-4 py-2 rounded-lg transition-colors">
              ...
              <div class="flex gap-3">
                ...
                <button @click="submitProducts" :disabled="loading" class="px-5 py-2 text-sm font-medium text-white bg-gray-900 hover:bg-gray-800 rounded-lg transition-colors shadow-sm disabled:opacity-50">
                  {{ loading ? 'Saving...' : (editingProduct ? 'Update Product' : `Save ${productForms.length > 1 ? productForms.length + ' Products' : 'Product'}`) }}
                </button>
              </div>
            </div>
          </div>
